# CLAUDE.md - High-Risk Operating Instructions

This file defines stable Claude Code / Cursor operating instructions for `igor-tp-reports-bot`.

## 1. Project purpose

`igor-tp-reports-bot` powers Igor's coach operating system in real workflows:

- TrainingPeaks reports and automation
- Telegram coach/student workflows
- billing/payment admin
- reminders, digests, and attention signals
- local runner workflows for sensitive TrainingPeaks actions

This is production-like coach infrastructure, not a toy app and not a generic demo repo.

Primary workflow must stay attended:
planning -> task prompt -> Cursor/Claude Code implementation -> checks -> human review -> manual commit/push.

No autonomous high-risk execution.

## 2. Risk level

This is a high-risk repository.

Reasons include:

- real student data
- real Telegram messages
- real billing/payment logic
- real TrainingPeaks plans/actions
- real production integrations
- local secrets and cookies may exist outside git

## 3. Hard safety rules

- Never commit unless explicitly asked.
- Never push unless explicitly asked.
- Never deploy unless explicitly asked.
- Never modify `.env*` files or print secrets.
- Never loosen auth, cron, token, webhook, Telegram, billing, or TrainingPeaks safety checks.
- Never run destructive database operations.
- Migrations: Claude applies them itself, under the protocol in the global CLAUDE.md
  («Применение миграций»). Additive DDL on the general command; `drop`/`delete`/
  `truncate`/column-type changes/anything that rewrites rows — separate explicit
  confirmation each time, with the affected row count stated first.
  Applied-or-not is verified by READING THE SCHEMA, never by the migrations list:
  on 2026-09-14 the repo holds 193 migration files against 37 rows in the database
  history, because most were applied by hand through the SQL editor.
- Never send Telegram messages unless explicitly requested.
- Never perform real TrainingPeaks mutations unless explicitly requested.
- Never change billing/payment allocation logic casually.
- Never broaden automation permissions without explicit approval.
- Never add autonomous execution for high-risk actions.
- Prefer dry-run, read-only, or prepare-only modes when available.

## 4. TrainingPeaks rules

TrainingPeaks mutations are high-risk.

Local Mac runner and browser/API automation must remain guarded. Prefer dry-run and verification-first behavior.

Fingerprints, source date checks, confidence gates, and human confirmation must not be weakened.

Move-workout automation must remain conservative.

If uncertain, ask for clarification or create a review case instead of executing.

Sensitive areas include:

- workout move actions
- source date inference
- TrainingPeaks API calls
- race/event scanner
- weekly report generation
- FIT parsing and segment analysis
- E-Predictor logic

## 5. Telegram rules

Telegram messages may contain private student context.

- Do not add broad group replies.
- Do not send messages automatically unless the task explicitly requires it.
- Do not weaken allowlists or coach-only protections.
- Preserve formality and tone logic when present.
- Prefer coach review before student-facing replies.
- Handle Business DM workflows and topic/group workflows carefully.

## 6. Billing rules

Billing and payment logic is sensitive.

- Do not auto-match payments more aggressively without explicit instruction.
- Do not modify payment allocation, payer identities, monthly payment state, or cron import behavior casually.
- Prefer manual review for ambiguous payments.
- Never print bank or payment secrets.

## 7. Development workflow

Claude/Cursor should:

- keep changes narrow
- inspect current code before editing
- avoid unrelated refactors
- preserve existing contracts
- update focused tests for behavior changes
- run the smallest relevant checks first
- run full checks when risk is high or broad files changed
- report exact files changed and commands run

Default checks:

- `npm run lint`
- `npm run build`
- `npm run check:runner-import-health`

Про третий. lint, tsc и build проверяют ТИПЫ, а боевые раннеры ломаются на РАЗРЕШЕНИИ МОДУЛЕЙ
в рантайме — это разные вещи, и первые вторых не видят. 11.08.2026 прод пролежал 3 часа:
коммит завёл `tools/trainingpeaks-export/tsconfig.json` без блока `paths`, tsx перестал
разворачивать алиас `@/`, и раннеры стали умирать на импорте `repository.ts` ДО первой строчки
работы. Все типовые проверки при этом были зелёными — поймать такое может только попытка
реально загрузиться, что чек и делает (обоими механизмами: tsx и `scripts/_alias-loader.mjs`).

Авария вдобавок была НЕВИДИМОЙ: `tp-heartbeat` импортирует тот же `repository.ts` и лёг вместе
с раннерами, поэтому в `trainingpeaks_cron_run_logs` не появилось даже строки `failed`, а пустая
таблица читается как «событий нет». Отсюда правило: пустой лог статусов — это НЕ «всё спокойно»,
это подозрение на смерть на импорте.

Чек идёт меньше секунды и ничего не пишет. Гонять ВСЕГДА, а не «когда кажется релевантным»:
тот коммит не выглядел опасным — это была уборка тайпчека.

When relevant:

- focused scripts/checks from `package.json`
- focused parser/TrainingPeaks/Billing checks
- TypeScript checks if available
- no real external mutations unless explicitly requested

## 8. Expected report format

Every task report should include:

1. Summary of changes
2. Files changed
3. Behavior before/after
4. Checks run and results
5. Safety confirmation:
   - no secrets touched
   - no push
   - no deploy
   - no unintended Telegram sends
   - no unintended TrainingPeaks mutations
   - no unintended billing imports or allocations
6. Commit hash only if explicitly asked to commit

## 9. What not to build

- no custom multi-agent runtime inside this repo
- no autonomous high-risk TrainingPeaks execution without human approval
- no broad Telegram auto-reply system without review gates
- no aggressive billing auto-matching without manual review
- no hidden background jobs that mutate production state without clear cron/auth guards
- no large dashboard/product expansion unless explicitly requested

## 10. Igor's preferred implementation style

- simple, narrow, production-minded changes
- no big rewrite unless explicitly requested
- avoid clever abstractions
- preserve working flows
- prefer readable code and practical checks
- communicate in clear summaries

## 11. Cowork (Claude desktop) рабочий процесс и контекст

Полная памятка: `docs/cowork-workflow.md`. Кратко:

- **Поток фичи:** Claude делает работу в отдельной ветке (worktree `wt-*`) + проверки (`npm run lint`, `npx tsc --noEmit`, относящиеся `check-*`). ЗАКАНЧИВАЕТ отчётом с единым блоком включения (см. ниже) и ЖДЁТ — сам НЕ пушит и НЕ вливает по своему решению. По явной команде Игоря («пушь»/«вливай») Claude делает всё сам: ff-merge в `main` (из папки, где `[main]`) → `git push origin main` → миграции (Supabase-MCP, по протоколу применения из глобального CLAUDE.md: аддитивное сразу, разрушительное — с отдельным «да») → и говорит, что осталось на Игоре (флаги, plist, деплой). Без PR-кликанья.
- **Push разрешён ТОЛЬКО по явной команде и ТОЛЬКО в `main` (fast-forward).** Автономно после задачи Claude не пушит НИКОГДА — заканчивает отчётом и ждёт. Хук `~/.claude/hooks/deny-guard.sh` + `~/.claude/settings.json` пропускают лишь обычный `git push origin main`; force / `--force-with-lease` / `-f` / push в чужие ветки / `--mirror` / `--delete` — заблокированы (git сам откажет в non-ff без force, так что ff гарантируется). Деплой (Vercel) и прод-миграции — по-прежнему через Игоря/по команде; миграции Claude кладёт файлом в `supabase/migrations/` и даёт в блоке включения SQL текстом.
- **Obsidian как контекст:** волт «Igor Second Brain» (`~/Library/Mobile Documents/iCloud~md~obsidian/Documents`) можно подключать как папку — Claude читает/ищет по заметкам (теги во фронтматтере, папки Agent-Hub / AI-Running-Coach / Ученики / Inbox) и складывает итоги задач туда же в том же формате (`ГГГГ-ММ-ДД - превью - хэш.md`, фронтматтер `type/source/category/tags`). Dev-summary по этому репозиторию → папка `Agent-Hub`.
- **Координация с Cursor:** пока активна сессия Claude, не давать Cursor делать git-операции (коммиты/checkout) — общий рабочий каталог, чужие коммиты «прилипают» к веткам Claude.

### Единый блок включения (в конце КАЖДОГО отчёта)

Всё, что нужно, чтобы фича заработала в бою, — ОДНИМ куском в конце отчёта, ничего не
раскидано по тексту. Порядок фиксирован:

1. **Миграции** — SQL текстом, готово к копипасту, в порядке применения (или «миграций нет»).
2. **Git** — точные команды: имя ветки буква-в-букву, хэш коммита, `cd` в папку где `[main]`,
   `git merge --ff-only <branch>`, `git push origin main`. (По команде «пушь»/«вливай» этот
   шаг Claude уже выполнил сам — тогда пишет «влито и запушено, подтверждено `git cherry`».)
3. **Флаги** — какие env выставить, `ИМЯ=значение`, и где (главный `.env.local`); что сейчас off.
4. **Plist / раннер** — что скопировать в `~/Library/LaunchAgents/`, `launchctl load`, `chmod +x` если нужно.
5. **Проверка** — чем убедиться, что включилось (что нажать / какой запрос / где посмотреть глазами).

По команде «пушь»/«вливай» Claude сам делает п.1 (через MCP, если доступен) и п.2; на Игоре
остаются флаги, plist и деплой — их Claude перечисляет явно как «осталось на тебе».

## 12. Git-протокол репозитория (СТРОГО — ветки не смешивать)

Одна задача = одна ветка = один worktree. Никогда не смешивать несвязанные
задачи в одной ветке.

- НИКОГДА `git add -A` / `git add .`. Только точечно свои файлы: `git add <пути>`.
  Перед каждым commit — `git status`; изменения не своей задачи НЕ добавлять.
- Старт задачи: ветвиться от свежего origin/main —
  `git fetch origin && git worktree add ../wt-<task> -b feature/<task> origin/main`
  (worktree в ~/, НЕ в /tmp). В начале назвать вслух: "Ветка: feature/<task>, worktree: <path>".
- ГДЕ ЛЕЖИТ main — выяснять КАЖДЫЙ РАЗ через `git worktree list` (искать строку с
  `[main]`) и делать все операции с main (checkout/merge/pull) ИМЕННО В ТОЙ папке.
  НЕ полагаться на путь из этого документа или из наряда: main переезжает между
  worktree, а текст устаревает молча. На 2026-07-14 main выгружен в
  /Users/igor/igor-tp-reports-bot; раньше он был в /Users/igor/igor-run-analysis, и
  та папка теперь в detached HEAD. Эта дата — не гарантия, а отметка: сверяйся с git.
  СИМПТОМ ПРОМАХА: `git merge --ff-only <branch>` в папке с detached HEAD проходит
  «успешно», но main не двигается — коммит тихо остаётся невлитым.
- Если оказался на чужой ветке или коммиты смешались — СТОП, показать
  `git log --oneline -10`, `git status`, `git branch --show-current`, спросить Игоря.
  НЕ делать reset --hard / rebase / merge между несвязанными ветками без явного
  подтверждения Игоря в текущем сообщении.
- "Доделай / влей / смёржи / пушь" = Claude выполняет САМ ВСЮ цепочку: commit в
  feature-ветке → `git fetch origin` → (при расхождении rebase + перегон проверок) →
  ff-merge в папке где `[main]` → `git push origin main` → миграции через Supabase-MCP
  по протоколу применения → и ОСТАНАВЛИВАЕТСЯ, отдав Игорю остаток (флаги, plist, деплой).
  Подтверждает влитие ФАКТОМ (`git cherry origin/main <branch>` пуст). Без такой команды
  Claude НЕ пушит и НЕ вливает — заканчивает отчётом с блоком включения и ждёт.
  Деплой (Vercel) Claude не запускает никогда — это всегда рука Игоря.
- ФАКТ ВЛИТИЯ ПОДТВЕРЖДАТЬ ЧЕРЕЗ `git cherry`, не «на глаз» и не по своему отчёту:
  `git cherry main <branch>` — пусто = все патчи ветки уже в main;
  `git cherry origin/main <branch>` — пусто = коммит реально ушёл пушем.
  Именно patch-equivalence, а НЕ `git rev-list --count main..branch`: счётчик предков
  врёт, когда патч попал в main через cherry-pick/rebase (хэш другой, содержимое то же).
  Правило уже трижды ловило тихую потерю: «влил в detached HEAD, main не сдвинулся»,
  «ветка на самом деле давно в main», «коммит не дошёл до origin».
- Перед ВЛИТИЕМ, а не только перед стартом, — `git fetch origin` и проверить, не уехал
  ли main вперёд (параллельные сессии вливаются в него на ходу). Если уехал: rebase
  своей ветки на свежий origin/main и ПЕРЕГНАТЬ проверки на объединённом коде —
  зелёные чеки на старой базе ничего не доказывают.

### Канонический каталог = main (раннеры читают ИМЕННО его)

`/Users/igor/igor-tp-reports-bot` — канонический каталог: тут лежат `.env.local` и
`node_modules`, и раннеры launchd (`cd $HOME/igor-tp-reports-bot`) исполняют код ИМЕННО
отсюда. Отсюда правила:

- Канонический каталог ВСЕГДА на `main`. НЕ парковать на нём feature-ветку — иначе раннеры
  молча гонят старый код. СИМПТОМ: влитый фикс не действует в бою (ловили на формате цели
  забега «цель 6599s» — раннер сидел на уже влитой, но припаркованной на каноническом
  каталоге ветке `feature/easy-pace-anchor`, и ни один влитый фикс не был активен).
- Вся feature-работа — в отдельных worktree `wt-*`, НЕ в каноническом каталоге.
- После КАЖДОГО влития — в каноническом `git -C /Users/igor/igor-tp-reports-bot pull --ff-only
  origin main`, чтобы раннеры сразу были на свежем коде.
- Перед отчётом «влито и работает» ПРОВЕРИТЬ фактом (не на глаз):
  `git -C /Users/igor/igor-tp-reports-bot rev-parse --abbrev-ref HEAD` = `main` И
  `git -C /Users/igor/igor-tp-reports-bot log --oneline -1` = свежий origin/main.

### Параллельные сессии (агентов несколько, репозиторий один)

Над репозиторием одновременно работают несколько агентов. Они коммитят в main, пока
идёт твой наряд. Исходить из того, что мир вокруг ветки СТОИТ, — нельзя.

ПРИЧИНА, ПОЧЕМУ ЭТО ЗАПИСАНО (одна сессия, 13–14.07): main сдвинулся 5 РАЗ по ходу
работы; один коммит был влит в detached HEAD, потому что агент пошёл по устаревшему
п.12 вместо `git worktree list`, и main не сдвинулся вообще; ещё один коммит целиком
ушёл в мусор как дубль — два агента независимо починили один и тот же баг.

- ОДИН УЗЕЛ КОДА — ОДНА СЕССИЯ. Не браться за задачу или файл, который может делать
  другой агент. Если наряд задевает чужой узел — сказать Игорю, а не «заодно поправить».
- ПЕРЕД СТАРТОМ наряда — `git log origin/main` (и `--oneline -15`): свериться, что
  задача ещё НЕ СДЕЛАНА. Цена пропуска: за одну сессию целый коммит ушёл в мусор как
  дубль — два агента независимо починили один и тот же баг.
- ПЕРЕД ВЛИТИЕМ — `git worktree list` (где реально [main]; документ устаревает) и
  `git cherry -v main <branch>` (применён ли ПАТЧ, а не «предок ли коммит»).
- `git diff main..branch` — ДВЕ точки: показывает, что реально уедет в main.
  НЕ три точки. `git diff main...branch` показывает, что ветка сделала ОТ ТОЧКИ
  РАСХОЖДЕНИЯ, — то есть её собственную историю, а не разницу с нынешним main.
  На этом легко обмануться: превзойдённая ветка, чья работа давно влита другим
  агентом, в три точки выглядит «уникальной». В две точки сразу видно обратное —
  что она ОТСТАЁТ от main и влитие СНЕСЛО БЫ более позднюю работу.
- ПОСЛЕ ВЛИТИЯ И ПУША — подтвердить ФАКТОМ, а не отчётом: `git fetch origin` и
  `git cherry origin/main <branch>` пуст = коммит реально в origin/main. Пока не
  подтверждено — задача не закрыта.
- Если в main появились коммиты, которых ты не делал, — НЕ считать их «чужой работой»
  и не действовать молча: показать Игорю (`git log --format='%h %an %ad · %s'`) и
  спросить. Они могут быть согласованными частями твоих же задач.

## 13. Навигация по коду через graphify

Индекс — graphify-out/graph.json (19M, gitignored).
GRAPH_REPORT.md — витрина верхнего уровня, НЕ индекс: многих реальных
символов в ней нет (проверено). Искать в graph.json.
Ни один из файлов не читать целиком.

1. Точный символ → файл и строка:
   grep -n -A3 '"label": "имяФункции()"' graphify-out/graph.json

2. Нестрогий поиск. Только -B, не -A: norm_label идёт последним полем
   в узле, поэтому -A покажет следующий узел, а не найденный.
   grep -n -B8 '"norm_label": "[^"]*фрагмент' graphify-out/graph.json | head -40
   Осторожно с подстроками: 'resolver' матчит resolveRawDayType.
   Для префикса — '"norm_label": "resolve'

3. Импакт-анализ (что ещё затронет правка). Взять "community": N
   у найденного узла, затем:
   grep -n -B5 '"community": N,' graphify-out/graph.json | grep source_file | sort -u | head -60
   Сообщества группируют по связям, а не по доменным фичам:
   в одной общине могут лежать nutrition и trainingpeaks. Использовать
   для "что сломается", а не для "какие файлы относятся к фиче Y".

4. Поиск по пути, когда точное имя символа неизвестно:
   grep -n -B3 -A5 '"source_file": "[^"]*фрагмент-пути' graphify-out/graph.json | head -60
   Полезно, когда поиск по norm_label даёт только scripts/ и package.json —
   значит в src/ модуль назван иначе.

Свежесть: built_at_commit в graph.json против git rev-parse HEAD.
Разошлись — граф отстал, использовать как подсказку, не как истину.

Отсутствие в графе НЕ означает отсутствие в проекте: данные и JSON
исключены через .graphifyignore, .sql-файлы не парсятся.

- Граф локальный: пакет `graphifyy` (две «y») строит его через tree-sitter AST — БЕЗ
  LLM, БЕЗ сети (пока не задан какой-либо `*_API_KEY`).
- Всё лежит в `graphify-out/` (в .gitignore — НЕ коммитить). Пересобирается сам после
  каждого коммита post-commit-хуком (фоновый, локальный); вручную — `graphify update .`.
- Выключить авто-пересборку: `export GRAPHIFY_HOOK_DISABLED=1`.

## 14. Вес ученицы в nutrition (правило тренера, 2026-07-28)

Вес для расчётов = новейший лог `nutrition_weight_logs` с `logged_at <= week_to` разбора;
НЕ сегодняшний новейший; профиль `nutrition_student_profiles.current_weight_kg` — только
если логов нет. `confirmed_by_coach` на выбор НЕ влияет (новейший лог берётся и
неподтверждённым — ученицы шлют вес через мини-апп, coach-confirm ставится редко).

- Единственная реализация — `src/features/nutrition/weight-resolution.ts`. Копипастить
  порядок по месту запрещено: до этой правки было ТРИ разных порядка в четырёх местах
  (разбор, план, карточка, список), и числа расходились внутри одного разбора.
  `check:nutrition-methodology-wiring` следит, чтобы копии не вернулись.
- Привязка к `week_to` — суть правила, а не деталь: без неё перегенерация старой недели
  пересчитает её по сегодняшнему весу и сохранённые числа поедут задним числом.
- Мусорный лог (вне 35–150 кг или скачок сверх допуска 3 % + 5 %/нед) не берётся, но и
  молча не подменяется: расчёт уходит на профиль И тренер получает видимую пометку в
  блоке «Вес (кг)» карточки. Пометка «лог не подтверждён» НЕ ставится — это норма.
- СИНХРОНИЗАЦИЯ: то же правило дублируется в Cowork-скилле `nutrition-weekly-report`
  (`SKILL.md` A5, `references/formulas.md` → «Coherence & bodyweight»). На 2026-07-28 скилл
  берёт `ORDER BY logged_at DESC LIMIT 1` без фильтра по `week_to` — то есть сегодняшний
  вес, и на перегенерации старой недели разойдётся с кодом. Меняешь правило — меняй в
  обоих местах.

## 15. Мини-апп: подтверждения — один примитив (/m/*)

Любое подтверждающее действие в `/m/*` (снять, отправить, пропустить, сгенерить, переключить)
идёт через один примитив: `src/app/m/_shared/use-optimistic-action.ts` (хук) +
`src/app/m/_shared/confirm-button.tsx` (кнопка). Не писать новый `fetch` + ручной
`busy`/`toast`-стейт на экран — подключиться к существующему.

- Хук `useOptimisticAction<TMeta>()` даёт `run(key, opts)` + `pending` (`Record<string, TMeta>`)
  + `isPending(key)`. Ключ — id строки/сущности: один и тот же ключ не даёт двум нажатиям по
  одной строке лететь параллельно (повтор игнорируется, пока первое в полёте); разные ключи
  никогда не блокируют друг друга.
- `run()` применяет `optimisticUpdate` СРАЗУ (до ответа сервера), затем ждёт `request()`; если
  `isSuccess(result)` — false, зовёт `rollback()` и `onFailure`. Сеть упала (`request` бросил) —
  тот же откат. Экран никогда не ждёт ответа, чтобы показать реакцию, и никогда не остаётся в
  наполовину применённом состоянии при ошибке.
- Haptic (`HapticFeedback` из Telegram WebView) — внутри примитива (`triggerHaptic`), не в
  каждом экране: тап всегда даёт вибро-отклик, даже когда сеть отвечает не мгновенно.
- Логика без React (`optimistic-action-core.ts`: `runOptimisticActionCore` +
  `createKeyedGate()`) — юнит-тесты в `optimistic-action-core.test.ts`
  (`npm run test:miniapp-optimistic-action`), без DOM/React.
- Пример подключения — `src/app/m/desk/reports-tab.tsx` + обработчики в `src/app/m/desk/page.tsx`
  (`sendReport`, `dismissReport`, `confirmShare`, …). Экран «Отчёты» (очередь разбора черновиков)
  переведён на примитив целиком (наряд 2026-09-16, «мгновенные подтверждения»).

**Запрещено в обработчиках подтверждений `/m/*`:** `router.refresh()`, `window.location.reload()`,
`revalidatePath()` на весь маршрут, серверный `"use server"`-экшен без `useTransition`. Точечная
инвалидация — через `optimisticUpdate`/`rollback` (патч конкретной сущности в локальном стейте),
не через полный рефетч страницы. Массовые операции («убрать все старше N дней», батч-генерация),
где результат заранее не предсказать, — рефетч ТОЛЬКО данных своей вкладки (как в
`generateBatch`/`bulkDismissOld`), не всего маршрута.

Диагноз наряда (2026-09-16): реальной перезагрузки/`revalidatePath` в `/m/*` не было НИ РАЗУ —
все экраны уже были на `fetch` + `setState`. Причиной ощущения «перезагружает» было отсутствие
оптимистичного апдейта и haptic (тап ничего не показывал до полного сетевого раунд-трипа), не
Next.js-навигация. Прежде чем чинить следующий подобный симптом — сверься с кодом, а не с
готовым списком гипотез.

## 16. Локальная админка (localadmin): production-режим, не dev

С 2026-09-17 `com.igor.coachos.localadmin` (порт 3005, канонический каталог) работает через
`next start` (production, `NEXT_DIST_DIR=.next-admin`), не через `npm run dev` — прежний dev-режим
разрастался по памяти на 16 ГБ Air. Слушает только `127.0.0.1` (не сеть) — доступ только с этого
Мака через `http://localhost:3005`.

- **Пересборка — только через `scripts/localadmin-rebuild.sh`**, никогда вручную `next build` в
  каноническом каталоге поверх живой `.next-admin`. Скрипт собирает в `.next-admin-new`, атомарно
  подменяет, делает health-check (200 + PID стабилен 20с) и откатывает сам при провале.
- **После мержа любых изменений, затрагивающих админку** (`src/app/admin/**`, `next.config.ts`,
  зависимости) — напомнить Игорю запустить `scripts/localadmin-rebuild.sh` в каноническом
  каталоге: без этого `next start` продолжит отдавать старую сборку молча.
- `distDir` берётся из `NEXT_DIST_DIR` (default `.next` — не трогает Vercel и обычный `npm run dev`
  где-либо ещё). `tsconfig.json` намеренно перечисляет ВСЕ варианты distDir сразу
  (`.next`, `.next-admin`, `.next-admin-new`, `.next-admin-old`) в `include` — иначе Next.js сам
  дописывает нужный путь при каждой сборке с новым `distDir`, и `git status` в каноническом
  каталоге переставал быть чистым после rebuild. По той же причине `next-env.d.ts` — в
  `.gitignore`: Next при каждой сборке переписывает в нём единственную `/// <reference path=...>`
  под текущий distDir (это замена значения, не добавление — «перечислить все варианты» тут не
  работает).
