...
## 13. Граф проекта (graphify) — grep по GRAPH_REPORT.md перед чтением исходников

При навигации по незнакомой части кода: СНАЧАЛА `grep` по
`graphify-out/GRAPH_REPORT.md`, чтобы найти нужный модуль и его связи (хаб-узлы,
соседи, сообщество), и только ПОТОМ читать исходники. Это резко снижает расход
контекста на переориентацию между сессиями.

ФАЙЛ ЦЕЛИКОМ НЕ ЧИТАТЬ — он большой (~124K). Использовать его как индекс через grep
(по имени модуля/функции/фичи), а не читать в контекст. Если файла нет — работай как
обычно.

- Граф локальный: пакет `graphifyy` (две «y») строит его через tree-sitter AST — БЕЗ
  LLM, БЕЗ сети, БЕЗ чтения doc/картинок (пока не задан какой-либо `*_API_KEY`).
  Данные учеников в граф не попадают.
...
- Точечные запросы вместо grep: `graphify query "<вопрос>"`, `graphify god-nodes`,
