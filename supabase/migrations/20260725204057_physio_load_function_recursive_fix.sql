...
begin
  ...
  obs as (
    select l.student_id, percentile_cont(0.995) within group (order by l.max_hr) as obs_max_hr
    from trainingpeaks_workout_laps l where l.max_hr between 90 and 230 group by 1
  ),
  obs_rest as (
  ...
  w as (
    ...
    where w.is_completed and w.workout_date >= current_date - p_days and w.workout_date <= current_date
  ),
  lap_agg as (
    ...
    where l.avg_hr between 70 and 220 group by 1
  ),
  per_workout as (
  ...
  bounds as (select student_id, min(day) as d0, current_date as d1 from daily group by 1),
  spine as (
  ...
  final as (
    select r.*,
      avg(trimp) over pw7  as a7,
      avg(trimp) over pw28 as a28,
      avg(trimp) over (partition by student_id order by day rows between 27 preceding and 7 preceding) as a21_prev,
      stddev_samp(trimp) over pw7 as sd7,
      sum(trimp) over pw7 as sum7
    from rec r
    window pw7  as (partition by student_id order by day rows between 6 preceding and current row),
           pw28 as (partition by student_id order by day rows between 27 preceding and current row)
  )
