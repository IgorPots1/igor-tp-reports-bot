-- Витрины опирались на current_date. База живёт по UTC, поэтому после полуночи
-- по Гринвичу радар становился пустым до следующего ночного пересчёта.
-- Теперь всё считается от ПОСЛЕДНЕЙ посчитанной даты алертов.
...
select
  a.student_id, a.student_name, a.alert_date,
...
returns jsonb language sql stable security definer set search_path = public as $$
  ...
  select jsonb_build_object(
    ...
    'flagged_today', (select count(distinct student_id) from physio_alerts, d
                       where alert_date = d.day and status='open' and severity>=2),
    'urgent_today', (select count(distinct student_id) from physio_alerts, d
                       where alert_date = d.day and status='open' and severity=3),
