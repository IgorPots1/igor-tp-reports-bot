-- v3. Три уровня защиты от мусора в данных:
--  (1) пульсовой гейт — настоящее максимальное усилие идёт на высоком пульсе
--      относительно личного максимума; отсекает велосипед/лыжи/битый GPS;
--  (2) отсев чистого сброса высоты;
--  (3) монотонность кривой — короткий отрезок не может быть быстрее длинного
--      более чем на 30%; кривая строится от длинного к короткому.
...
begin
  ...
  create temp table if not exists _cand (
    ...
    lap_from int, lap_to int, rk int, n_workouts int
  ) on commit drop;
  delete from _cand;
  ...
  hrmax as (
    select l.student_id,
           percentile_cont(0.99) within group (order by l.max_hr) as hr_peak
  ...
  valid as (
    ...
    where speed_mps between 1.5 and
      ...
      -- пульсовой гейт: усилие должно идти на высоком проценте личного максимума
  ...
  -- построение монотонной кривой от длинного отрезка к короткому
  for v_student in select distinct student_id, student_name from _cand loop
    ...
    foreach v_bucket in array v_buckets loop
      if v_prev is null then
        ...
        -- быстрее длинного отрезка, но не более чем на 30%
        select * into v_pick from _cand c
        where c.student_id = v_student.student_id and c.bucket = v_bucket
