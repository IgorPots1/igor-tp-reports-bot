-- Подгонка модели критической скорости d = CS*t + D' по точкам кривой,
-- зоны от CS, VDOT по Daniels-Gilbert, прогноз результата с поправкой
-- на durability (за пределами 30 мин чистая CS-модель систематически врёт).
...
begin
  ...
  with fit as (
    select student_id,
           ...
           regr_slope(distance_m, duration_s)     as cs,
           regr_intercept(distance_m, duration_s) as dprime,
           regr_r2(distance_m, duration_s)        as r2,
           count(*)::int as n,
  ...
  dur as (
    select student_id,
           percentile_cont(0.5) within group (order by decline_per_hour_pct) as dur_idx,
  ...
  base as (
    select f.*, d.dur_idx, d.n_samples, z.lthr, z.max_hr, z.rest_hr, z.thr_speed,
           s.trainingpeaks_athlete_id,
           -- дистанция, достижимая за 30 мин по модели — якорь для длинных прогнозов
           (f.cs * 1800 + f.dprime) as d30,
           -- показатель Ригеля, скорректированный на durability
           least(1.14, greatest(1.03,
             1.04 + 0.008 * least(12, greatest(0, coalesce(d.dur_idx, 5)))))::numeric as riegel_e
    from fit f
    left join dur d on d.student_id = f.student_id
    left join zsnap z on z.student_id = f.student_id
    left join (select id, (regexp_match(trainingpeaks_athlete_url, '(\d+)'))[1]::bigint as trainingpeaks_athlete_id
  ...
  predicted as (
    select s.*,
      (select jsonb_object_agg(dist.label, jsonb_build_object(
          'distance_m', dist.d,
          'seconds', round(t.sec),
          'time', to_char((t.sec || ' seconds')::interval, 'HH24:MI:SS'),
          'pace_s_km', round(t.sec / (dist.d/1000.0)),
          'method', t.method))
       from (values ('d1500',1500.0),('d3000',3000.0),('d5000',5000.0),
                    ('d10000',10000.0),('half',21097.5),('marathon',42195.0)) as dist(label,d)
       cross join lateral (
  ...
  withvdot as (
    select p.*,
      (select case when pct > 0 then vo2/pct end
       ...
       lateral (select 0.8 + 0.1894393*exp(-0.012778*q.tmin)
                     + 0.2989558*exp(-0.1932605*q.tmin) as pct) pc
      ) as vdot
    from predicted p
  )
