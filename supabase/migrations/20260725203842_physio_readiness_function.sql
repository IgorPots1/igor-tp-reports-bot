-- Ежедневная готовность. Модель Plews/Altini: ln(RMSSD) сравнивается не с
-- вчерашним днём, а со скользящей базой +/- SWC (0.5 SD) — иначе шум съедает сигнал.
...
begin
  ...
  ln as (
    select d.*, case when hrv > 0 then ln(hrv) end as ln_rmssd from daily d
  ),
  rolled as (
    select l.*,
