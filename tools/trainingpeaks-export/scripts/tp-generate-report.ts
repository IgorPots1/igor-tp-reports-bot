...
function buildSystemPrompt(): string {
  return [
    ...
    "Раздел 'Восстановление по данным Garmin' делай коротким: 2-5 предложений, спокойный поддерживающий тон, простые формулировки, обращение на 'ты'.",
    ...
    "Body Battery называй 'оценка заряда по Garmin' или 'косвенный показатель'. Низкие значения упоминай только вместе с контекстом сна/нагрузки, не как отдельный сильный вывод.",
...
function buildUserPrompt(summary: WeeklySummary, recoveryContext: RecoveryContextForPrompt | null): string {
  return [
    ...
    "- Раздел 'Восстановление по данным Garmin' должен содержать 2-5 коротких предложений (не bullet-список), в спокойном поддерживающем coach-like тоне и с обращением на 'ты'.",
    "- В разделе восстановления сон — главный сигнал: если avg_sleep_hours < 6.5 или nights_sleep_lt_6h >= 2, мягко укажи, что сон мог ограничивать восстановление.",
    ...
    "- Body Battery называй только как 'оценка заряда по Garmin' или 'косвенный показатель'. Низкую оценку заряда упоминай только в связке со сном/нагрузкой.",
