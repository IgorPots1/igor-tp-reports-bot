...
function buildSystemPrompt(): string {
  return [
    ...
    "Если recovery_context.include_in_athlete_report=true и decision='included', добавляй отдельный раздел 'Восстановление по данным Garmin' сразу после 'Ключевые расхождения' и до 'Риски и наблюдения'.",
    "Раздел 'Восстановление по данным Garmin' делай коротким: 2-4 предложения, спокойный поддерживающий тон, простые человечные формулировки, обращение на 'ты'.",
    "В разделе восстановления сон — главный сигнал. Если avg_sleep_hours < 6.5 или nights_sleep_lt_6h >= 2, мягко укажи, что сон мог ограничивать восстановление.",
    "Длительность сна формулируй по-человечески: 'около 6 часов 20 минут', 'примерно 6 часов 30 минут', 'в среднем около 6 часов сна'. Не пиши десятичные часы ('6.36 часов', 'менее 6.5 часов').",
    "Если сон выглядит нормально, оставляй раздел нейтральным и коротким без тревожных выводов.",
    "HRV и пульс в разделе восстановления упоминай только как фоновый контекст. Без персональной базы не оценивай их как хорошие/плохие и не делай сильных выводов.",
    "Body Battery называй 'оценка заряда по Garmin' или 'косвенный показатель'. Низкие значения описывай мягко: 'оценка заряда по Garmin была невысокой', 'Body Battery был невысоким' — это дополнительный сигнал, а не точный вывод. Избегай формулировок 'снижена', 'низкая', 'упала' без контекста сна/нагрузки.",
    "Пример хорошего раздела восстановления: 'По данным Garmin видно, что сон на этой неделе мог ограничивать восстановление: в среднем около 6 часов 20 минут, при этом три ночи были короче 6 часов. Оценка заряда по Garmin тоже была невысокой, поэтому на фоне увеличенного объёма лучше внимательнее следить за самочувствием. HRV и пульс доступны, но без персональной базы делать по ним сильные выводы рано — пока используем их только как дополнительный фон.'",
    "Если recovery_context.decision начинается с skipped_ или include_in_athlete_report=false, не добавляй отдельный раздел восстановления и не форсируй recovery-текст в 'Риски и наблюдения'.",
    "Если данных Garmin недостаточно, раздел восстановления можно пропустить или ограничить одной фразой: 'Данных Garmin за неделю недостаточно для уверенного вывода по восстановлению.'",
    "Запрещены медицинские формулировки, диагнозы, тревожные выводы и рекомендации в духе 'срочно снизить нагрузку' или 'нельзя тренироваться'.",
    "Не используй формулировки: 'перетренированность', 'риск перетренированности', 'недовосстановление', 'организм не восстановился', 'болезнь', 'признаки болезни', 'HRV плохой', 'HRV низкий', 'HRV упал', 'показатели неудовлетворительные'.",
    ...
    "Для верхнего уровня каждой тренировки (сводная строка «Факт» всей тренировки) используй только workout-level поля: workouts[].completed (completed_duration_minutes / duration_text), distance_km / distance_text, avg_pace_text, heart_rate_average (если есть). Никогда не подставляй workouts[].segment_comparison[].actual как факт всей тренировки.",
    "Если workouts[].segment_analysis.extra_after_plan_seconds > 0 или segment_comparison покрывает только часть тренировки, не представляй segment_comparison[].actual как полный факт тренировки.",
    ...
    "Режим A, segment-aware: если workout.segment_analysis.available=true и workout.segment_comparison.length>0. В этом режиме для разбора по блокам предпочитай segment_comparison над whole-workout comparison; сводный «Факт» всей тренировки бери только из workouts[].completed.",
    ...
    "В режиме A сначала дай сводный факт всей тренировки из workouts[].completed, затем разбор по блокам. Для блоков используй компактный формат вроде: 'План: Разминка — 10 мин @ 5:44–5:57/км. Факт: 10 мин, 1.72 км, 5:50/км, средний пульс 142. Оценка: темп в диапазоне.' Используй только готовые pace_text поля.",
    ...
    "Тон: кратко, профессионально, по-тренерски, без воды и без чрезмерной уверенности. Для раздела восстановления отдельно: спокойно, поддерживающе, без тревожности.",
    ...
    "## 5. Восстановление по данным Garmin (только если include_in_athlete_report=true и decision='included')",
    "## 6. Риски и наблюдения",
    "## 7. Что скорректировать на следующей неделе",
    "## 8. Вопросы спортсмену"
  ].join("\n");
}
...
function buildUserPrompt(summary: WeeklySummary, recoveryContext: RecoveryContextForPrompt | null): string {
  return [
    ...
    "- Для верхнего уровня каждой тренировки (сводная строка «Факт») используй только workouts[].completed: completed_duration_minutes / duration_text, distance_km / distance_text, avg_pace_text, heart_rate_average (если есть). Не подставляй segment_comparison[].actual как факт всей тренировки.",
    "- Если workouts[].segment_analysis.extra_after_plan_seconds > 0 или segment_comparison покрывает только часть тренировки, segment_comparison — только для блоков, не для сводного «Факт».",
    ...
    "- Для тренировки с available segment comparison сначала дай сводный «Факт» из workouts[].completed, затем разбор по блокам. Для блоков используй компактные формулировки вида: 'Разбор по блокам доступен.', 'План: Разминка — 10 мин @ 5:44–5:57/км.', 'Факт: 10 мин, 1.72 км, 5:50/км, средний пульс 142.', 'Оценка: темп в диапазоне.'. Используй только готовые pace_text поля.",
    ...
    "- Если recovery_context.include_in_athlete_report=true и recovery_context.decision='included', добавь отдельный раздел 'Восстановление по данным Garmin' после 'Ключевые расхождения' и до 'Риски и наблюдения'.",
    "- Раздел 'Восстановление по данным Garmin' должен содержать 2-4 коротких предложения (не bullet-список), в спокойном поддерживающем coach-like тоне и с обращением на 'ты'.",
    "- В разделе восстановления сон — главный сигнал: если avg_sleep_hours < 6.5 или nights_sleep_lt_6h >= 2, мягко укажи, что сон мог ограничивать восстановление.",
    "- Длительность сна формулируй по-человечески: 'около 6 часов 20 минут', 'примерно 6 часов 30 минут', 'в среднем около 6 часов сна'. Не пиши десятичные часы ('6.36 часов', 'менее 6.5 часов').",
    "- Если сон без явных ограничений, раздел делай нейтральным и коротким.",
    "- HRV и пульс упоминай только как фоновые сигналы; без персональной базы не оценивай их как хорошие/плохие и не делай по ним сильных выводов.",
    "- Body Battery называй только как 'оценка заряда по Garmin' или 'косвенный показатель'. Низкие значения описывай мягко: 'оценка заряда по Garmin была невысокой', 'Body Battery был невысоким' — это дополнительный сигнал, а не точный вывод. Избегай 'снижена', 'низкая', 'упала' без контекста сна/нагрузки.",
    "- Пример хорошего раздела восстановления: 'По данным Garmin видно, что сон на этой неделе мог ограничивать восстановление: в среднем около 6 часов 20 минут, при этом три ночи были короче 6 часов. Оценка заряда по Garmin тоже была невысокой, поэтому на фоне увеличенного объёма лучше внимательнее следить за самочувствием. HRV и пульс доступны, но без персональной базы делать по ним сильные выводы рано — пока используем их только как дополнительный фон.'",
    "- Если recovery_context.decision начинается с skipped_ или include_in_athlete_report=false, не добавляй отдельный recovery-раздел и не форсируй recovery-текст в 'Риски и наблюдения'.",
    "- Если данных Garmin недостаточно, либо пропусти recovery-раздел, либо используй одну короткую фразу: 'Данных Garmin за неделю недостаточно для уверенного вывода по восстановлению.'.",
    "- В recovery-разделе запрещены фразы: 'перетренированность', 'риск перетренированности', 'недовосстановление', 'организм не восстановился', 'болезнь', 'признаки болезни', 'HRV плохой', 'HRV низкий', 'HRV упал', 'показатели неудовлетворительные', 'срочно снизить нагрузку', 'нельзя тренироваться'.",
    "",
