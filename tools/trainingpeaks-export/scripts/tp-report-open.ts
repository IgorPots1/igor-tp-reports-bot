...
async function openWithDefaultMacOsApp(targetPath: string): Promise<void> {
  if (process.platform !== "darwin") {
    throw new Error("tp-report-open currently supports macOS only.");
  }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on("exit", (code, signal) => {
      if (signal) {
        reject(new Error(`open exited from signal ${signal}.`));
        return;
      }

      if (code !== 0) {
        reject(new Error(`open failed with exit code ${code}.`));
        return;
      }
...
async function main(): Promise<void> {
  ...
  console.log("Opening report draft...");
  await openWithDefaultMacOsApp(reportPath);
}
