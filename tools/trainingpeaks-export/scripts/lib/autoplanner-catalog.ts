...
/**
 * ОБВЯЗКА ПО ПРОПОРЦИИ, А НЕ КОНСТАНТОЙ [18.09.2026].
 *
 * Разминка и заминка в каталоге зашиты числом: у steady_tempo 15/10, у thr_*
 * 12/12. То есть ЛЮБОЙ формат стоит 24–25 минут ещё до первой минуты работы, и
 * самая короткая качественная выходит 35 минут при десяти минутах работы. У
 * человека с недельным объёмом 70–85 бюджет качественной выходит 22–32, и не
 * помещается НИЧЕГО: неделя за неделей оставалась без работы с причиной
 * no_preset_fits_week.
 *
 * ПРОПОРЦИЯ ВЗЯТА ИЗ КАРТОЧЕК ТРЕНЕРА, НЕ ПРИДУМАНА. В интервальной, которую
 * Игорь написал рукой: разминка 21 минута из 64 (5 шагом + 10 лёгкого +
 * 3 ускорение + 3 спокойного), заминка 5. Треть сессии на разминку и примерно
 ...
 *
 * ПРИМЕНЯЕТСЯ ТОЛЬКО ПРИ volumeIsReported, ровно как пол лёгкой
 * (EASY_FLOOR_REPORTED_MIN). Ростер и ветка истории Intervals этот флаг не
 * ставят никогда — их обвязка остаётся каталожной.
 */
...
export function withScaledWarmup(p: QualityPreset): QualityPreset {
  ...
  return { ...p, warmupMinutes: warm, cooldownMinutes: cool, athleteLevelMin: null };
}
