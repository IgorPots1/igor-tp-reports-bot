...
export type AthleteAnchors = {
  ...
  /**
   * Разрешено ли назначать качество ПО УСИЛИЮ, когда порога нет.
   *
   * ФЛАГ, А НЕ ОБЩЕЕ ПРАВИЛО. У ростера TrainingPeaks отказ «нет порога»
   * означает «сначала поставь порог», и менять это нельзя: недели ростера
   * обязаны остаться прежними до байта. Флаг включает только ветка Intervals,
   * где порога неоткуда взять в принципе.
   *
   * РЕЗОЛВЕР ЕГО НЕ ЧИТАЕТ И ПО-ПРЕЖНЕМУ ОТКАЗЫВАЕТ: его дело — перевести
   * намерение в темпы, а темпов тут нет. Решение «тогда по усилию» принимает
   * сборщик сессии, у которого под рукой пресет с его RPE.
   */
