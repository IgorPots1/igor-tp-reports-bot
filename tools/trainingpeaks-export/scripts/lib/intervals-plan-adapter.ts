...
export function buildDraftFromOnboarding(
  ...
  let baseAerobic = Math.max(0, Math.round(start.medianWeeklyMinutes));
  ...
  if (answers.maxSessionMinutes !== null && answers.daysPerWeek > 0) {
    ...
    /**
     * ПОТОЛОК НИЖЕ БАЗЫ — ЭТО НАДО СКАЗАТЬ, А НЕ ПРОГЛОТИТЬ [22.09.2026].
     *
     * У Валентины сошлось так: объём со слов 185 мин в неделю, но не длиннее
     * 60 минут за раз и три дня — то есть вместимость 180. База оказалась ВЫШЕ
     * потолка, и цикл на двенадцать недель встал: каждая неделя упиралась в
     * 180, шаг ×1.06 не давал ничего, и тренер видел двенадцать одинаковых
     * недель без единого слова о причине.
     *
     * База приводится к вместимости: план на объём, который не помещается в
     * собственные ограничения человека, невыполним предсказуемо. И это
     * называется вслух — роста в таком цикле не будет, пока не вырастет число
     * дней или потолок одной тренировки.
     */
    if (baseAerobic > weeklyCapacity) {
      notes.push(
        `⚠️ РОСТА В ЭТОМ ЦИКЛЕ НЕ БУДЕТ. Объём со слов ${baseAerobic} мин в неделю не помещается ` +
          `в собственные ограничения: не длиннее ${answers.maxSessionMinutes} мин за раз × ` +
          `${answers.daysPerWeek} дн = ${weeklyCapacity} мин. База приведена к ${weeklyCapacity}. ` +
          "Чтобы цикл рос, нужен либо ещё один беговой день, либо более длинная тренировка."
      );
      baseAerobic = weeklyCapacity;
    }
  }
