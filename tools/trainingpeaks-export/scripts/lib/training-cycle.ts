...
/**
 * develop — РАЗВИТИЕ БЕЗ СТАРТА [решение 15.09.2026]. Человек уже бегает
 * регулярно, старта нет, хочет улучшать результаты.
 *
 * ПОЧЕМУ ОТДЕЛЬНЫЙ ТИП, А НЕ ПРАВКА maintenance. maintenance работает на ростере
 * TrainingPeaks и означает там ровно «держим объём, не растим»: у него шаг ×1.00
 * и намеренно нет разгрузок. Дописать туда рост значило бы поменять поведение
 * живого ростера ради нового сегмента. У develop своё поведение: растёт как
 * подготовка, но без обратного отсчёта и без подводки.
 */
export type CycleIntent = "5k" | "10k" | "half" | "marathon" | "maintenance" | "develop";
 ...
 */
export type CycleMode = "подготовка" | "подвод к старту в текущей форме" | "поддержание" | "развитие";
...
export const LENGTH_WEEKS: Record<CycleIntent, number> = {
  ...
  // ДВЕНАДЦАТЬ, А НЕ ВОСЕМЬ. Восемь недель maintenance — это длина отрезка, на
  // котором ничего не растёт, и удерживать его дольше незачем. У развития
  // другая арифметика: при разгрузке каждую четвёртую неделю в восьми неделях
  // остаётся шесть ростовых, и половина цикла уходит на разгон. Двенадцать
  // дают девять ростовых недель и три разгрузки — блок, который реально
  // сдвигает объём, и при этом не длиннее, чем человек способен планировать.
  develop: 12,
};
...
export const TAPER_PROFILE: Record<CycleIntent, TaperWeek[]> = {
  ...
  maintenance: [],
  // Подводки нет: подводят К СТАРТУ, а старта здесь нет. Цикл заканчивается
  // разгрузкой, после которой собирается следующий блок.
  develop: [],
};
