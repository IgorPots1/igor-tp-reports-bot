...
export function buildWeek(a: AthleteAnchors, env: Envelope, cat: Catalog, weekStart: string, hasActiveIllness: boolean,
  ...
  //
  // volumeIsReported — ЯВНОЕ ИСКЛЮЧЕНИЕ ИЗ ЭТОГО ПОРОГА [решение Игоря,
  // 17.09.2026]. Порог в 4 недели — тренерское решение о том, со скольких
  // недель ЗНАКОМСТВА машине разрешено планировать (см. комментарий у
  // MIN_WEEKS_FOR_ENVELOPE), не про точность самого числа. Для человека без
  // подключаемых часов (весь сегмент вроде Honor) недель не будет никогда —
  // ждать их значит никогда не собрать план. Объём со слов даёт цикл ниже
  // доверием (широкие полосы — см. startingPointFromAnswers), но собирает.
  if (env.weeksObserved < MIN_WEEKS_FOR_ENVELOPE && !env.volumeIsReported) {
  ...
  if (env.weeksObserved < MIN_WEEKS_FOR_ENVELOPE && env.volumeIsReported) {
    notes.push(
      "КОНВЕРТ ОБЪЁМА — СО СЛОВ, НЕ ИЗМЕРЕН. Недель наблюдения нет вообще: тренер назвал " +
        "объём сам. Полосы личного пола/цели/потолка лёгкой намеренно широкие, а не из " +
        "перцентилей по практике — проверяйте эту неделю внимательнее обычного."
    );
  }
