...
const MUTATIONS: Mutation[] = [
  ...
  { id: "growth-manual-step-ignored", file: R, find: "  const stepAerobic = num(r.step_aerobic_manual, num(r.step_aerobic, STEP_AEROBIC));", replace: "  const stepAerobic = num(r.step_aerobic, STEP_AEROBIC);", what: "ручной шаг роста игнорируется" },
  { id: "growth-target-ignored", file: R, find: "    if (target != null) {\n      const step = stepForTargetPeak(draft, start, wantWeeks, target);", replace: "    if (false && target != null) {\n      const step = stepForTargetPeak(draft, start, wantWeeks, target);", what: "целевой объём к пику игнорируется" },
  { id: "growth-step-beats-target", file: R, find: "      draft = { ...draft, stepAerobic: step, stepQuality: step };", replace: "      void step;", what: "цель не перебивает ручной шаг" },
  { id: "growth-peak-cap-frozen", file: R, find: "  const peakCap = target != null\n    ? Math.round(target * (1 - qShare))", replace: "  const peakCap = false && target != null\n    ? Math.round(target * (1 - qShare))", what: "потолок пика не едет за целью — цель молча недостижима" },
  { id: "growth-no-limit", file: C, find: "  if (peakOf(MAX_SINGLE_STEP) <= targetWeeklyMin) return MAX_SINGLE_STEP;", replace: "  if (peakOf(MAX_SINGLE_STEP) <= targetWeeklyMin) return 1.9;", what: "недостижимая цель разгоняет цикл выше предела одного перехода" },
  { id: "share-clamp-growth-off", file: C, find: "    const clG = clampShare(a, qBeforeClamp, draft.ownSharePct, draft.peakCapQualityMin);", replace: "    const clG = { q: qBeforeClamp, note: null };", what: "зажим доли качества не применяется в НЕДЕЛЕ РОСТА" },
  ...
  { id: "days-long-from-all", file: W, find: "    const confident = confidentLongDay(hist.long);", replace: "    const confident = daysByPreference(hist.all, hist.all)[0];", what: "день длительной снова берётся из ОБЩЕЙ гистограммы" },
  { id: "long-day-no-concentration", file: P, find: "  return hist[best] / total >= LONG_DAY_MIN_CONCENTRATION ? best : null;", replace: "  return best;", what: "порог концентрации снят — слабая гистограмма снова назначает будний день" },
  { id: "long-day-always-fallback", file: P, find: "  return hist[best] / total >= LONG_DAY_MIN_CONCENTRATION ? best : null;", replace: "  void best; return null;", what: "личный день не берётся никогда — всем воскресенье" },
  { id: "long-day-fallback-monday", file: P, find: "export const LONG_DAY_FALLBACK: number = 6; // воскресенье", replace: "export const LONG_DAY_FALLBACK: number = 0; // мутация: понедельник", what: "запасной день длительной — понедельник вместо воскресенья" },
  { id: "long-tie-counts", file: P, find: "  if (nq.length > 1 && nq[0].min - nq[1].min < LONG_DAY_MIN_GAP_MIN) return null;", replace: "  void LONG_DAY_MIN_GAP_MIN;", what: "ничья по минутам снова считается наблюдением дня длительной" },
  { id: "long-gap-too-strict", file: P, find: "  if (nq.length > 1 && nq[0].min - nq[1].min < LONG_DAY_MIN_GAP_MIN) return null;", replace: "  if (nq.length > 1 && nq[0].min - nq[1].min < 100) return null;", what: "порог ничьей задран — настоящие длительные перестают быть наблюдениями" },
