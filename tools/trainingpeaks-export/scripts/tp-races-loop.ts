...
async function runOnceRequest(): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const child = spawn("npm", ["run", "tp-races-requests-once"], {
      stdio: "inherit",
      env: process.env,
      shell: false,
    });
    ...
    child.on("exit", (code) => {
      ...
      reject(new Error(`Command failed: npm run tp-races-requests-once (exit ${String(code)})`));
    });
  });
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `[tp-races-loop] started interval=${options.intervalSeconds}s once=${options.once ? "yes" : "no"}`
  );
  ...
  const runScheduledTick = async (): Promise<void> => {
    if (isRunningTick) {
      console.log("[tp-races-loop] previous tick still running, skipping this interval");
      return;
    }
    ...
    tickNo += 1;
    try {
      console.log(`[tp-races-loop] tick ${tickNo} started`);
      await runOnceRequest();
      console.log(`[tp-races-loop] tick ${tickNo} finished`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error(`[tp-races-loop] tick ${tickNo} failed: ${message}`);
    } finally {
      isRunningTick = false;
    }
  };
...
main().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`[tp-races-loop] fatal error: ${message}`);
  process.exitCode = 1;
});
