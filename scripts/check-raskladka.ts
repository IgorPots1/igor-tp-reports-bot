...
function check(course: Course, label: string, target: number, finish: Finish) {
  ...
  if (finish === "kick") {
    ...
    // Пять секунд — округление, а не разгон: на 44:00 такой «разгон» давал ту же
    // раскладку, что и «Ровный», и кнопка выглядела сломанной.
    assert.ok(kick >= 10, `${what}: разгон ${kick} с, это округление, а не разгон`);
  } else {
...
// Заморожены 25.09.2026. Разгон считается от самой медленной рабочей полосы,
// лежит между 10 и 15 с, и среди допустимых выбирается наименьший.
{
  const c = COURSES["10"];
  const expected: Record<string, string[]> = {
    "38:00": ["3:50", "3:50", "3:40"],
    "44:00": ["4:20", "4:30", "4:15"],
