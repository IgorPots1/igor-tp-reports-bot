...
async function run(): Promise<void> {
  ...
  // Rest 56 kg is now 1800 kcal (the macros, not a flat 35 kcal/kg table) → ±50 display band.
  assert.match(planDraft, /~1750-1850 ккал/, "weekly plan draft should display practical kcal range for rest day");
  assert.doesNotMatch(planDraft, /~1800 ккал/, "weekly plan draft should not expose the raw point target");
