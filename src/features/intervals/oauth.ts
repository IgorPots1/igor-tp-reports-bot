 ...
 *
 * ACTIVITY:READ   — сами тренировки и их ряды, ради этого всё и затевается.
 * ACTIVITY:WRITE  — чтобы позже класть разбор в комментарий к тренировке.
 * WELLNESS:READ   — пульс покоя, сон, вес: контекст «почему далось тяжело».
 * CALENDAR        — план в календаре ученика (следующий шаг, но права просим
 *                   сразу: повторное согласие — это ещё один экран, на котором
 *                   человек отваливается).
 *
 * Список ровно из наряда. Лишнего не просим: каждая строчка в окне согласия —
 * это повод не нажать «Разрешить».
 */
