...
/**
 * РАЗВИЛКА ЦЕЛИ. Варианты названы ПОСЛЕДСТВИЕМ, а не намерением.
 *
 * Прежние формулировки («просто бегать регулярно») описывали настроение, а
 * выбирали при этом поведение генератора на два месяца вперёд: goal_kind
 * 'regular' уходит в intent 'maintenance', а это восемь недель с шагом ×1.00 —
 * ни роста объёма, ни единой качественной тренировки. Человек соглашался на
 * «просто бегать» и получал план, в котором ничего не происходит, не зная,
 * что выбрал именно это.
 *
 * Поэтому подпись каждого варианта говорит, ЧТО БУДЕТ, а не что человек
 * чувствует. Третий вариант остаётся: тем, кому правда ничего менять не надо,
 * он честен — но теперь он честен вслух.
 */
export type GoalKind = "race" | "improve" | "regular";

export const GOAL_OPTIONS: Array<{ code: GoalKind; labelRu: string; hintRu: string }> = [
  {
    code: "race",
    labelRu: "Готовлюсь к старту",
    hintRu: "план соберётся под дату и дистанцию: объём растёт, ближе к старту подводка",
  },
  {
    code: "improve",
    labelRu: "Хочу бегать больше и лучше",
    hintRu: "объём растёт каждую неделю, появляется работа на скорость, каждая четвёртая неделя разгрузочная",
  },
  {
    code: "regular",
    labelRu: "Хочу сохранить нынешнюю форму, ничего не меняя",
    hintRu: "объём останется тем же все восемь недель: ни роста, ни ускорений, только регулярность",
  },
];
