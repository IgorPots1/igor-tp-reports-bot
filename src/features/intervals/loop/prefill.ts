...
export const NEVER_IN_APP_FORM: readonly PrefillableField[] = [
  ...
  "daysPerWeek",
  "selfReportedWeeklyMinutes",
  // canRunContinuously ВЕРНУЛСЯ В ФОРМУ [15.09.2026] и потому из этого списка
  // убран. Раньше его всегда закрывало предзаполнение из бота; теперь заведение
  // идёт в один шаг, и тренер за человека не отвечает. Это факт про сегодняшний
  // день, который она знает лучше всех и отвечает за полминуты.
  //
  // Предзаполнение поля никуда не делось: из скрипта его по-прежнему можно
  // задать, когда тренер действительно видел человека в деле, и тогда вопрос в
  // форме не появится. Так работает любое предзаданное поле.
  "healthLimits",
  "experienceNote",
];
