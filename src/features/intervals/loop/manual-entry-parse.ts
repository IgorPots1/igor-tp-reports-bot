 ...
 * «1.05» — это час пять, см. ниже), «1ч05», «1 05».
 *
 ...
 *
 * ── ПОЧЕМУ ЭТОГО ПРАВИЛА НЕ ХВАТИЛО [23.09.2026] ────────────────────────────
 *
 * Ученица пробежала час шесть и написала «106». Форма честно поняла сто шесть
 * минут, записала 1:46, и расхождение всплыло только через два дня, когда
 * тренер спросил её про цифры.
 *
 * Виновато не это правило, а СОСЕДНЕЕ ПОЛЕ. В темпе «812» означает 8:12 —
 * цифры подряд там и есть запись через разделитель, это сделано намеренно и
 * написано у поля. Человек выучил приём на одном поле и применил на соседнем,
 * где он значит противоположное. Из двух правил каждое по отдельности верное,
 * а вместе они учат разному.
 *
 * Менять правило на «106 = 1:06» значит менять одну догадку на другую и ломать
 * честный случай «105 минут». Поэтому правило осталось, а форма теперь ПОКАЗЫВАЕТ,
 * как поняла, словами и сразу (describeDurationRu), и у трёхзначного числа
 * предлагает второе прочтение (durationAlternativeRu). Догадываться должен
 * человек, а не программа, — но он должен видеть, о чём догадывается.
 */
export function parseDurationMinutes(raw: string): ParseResult {
  /**
   * ПРОБЕЛ МЕЖДУ ЦИФРАМИ — ЭТО РАЗДЕЛИТЕЛЬ, А НЕ НИЧТО. «1 06» это час шесть, а
   * не сто шесть: никто не пишет трёхзначное число с пробелом посередине.
   * Проверяем ДО normalize, который пробелы съедает.
   */
  const spaced = raw.trim().match(/^(\d{1,2})\s+(\d{1,2})$/u);
  if (spaced) {
    return parseDurationMinutes(`${spaced[1]}:${spaced[2]}`);
  }
...
/**
 * Как разобранное время читается словами: «1 ч 46 мин», «40 мин», «2 ч».
 *
 * Это и есть предпросмотр у поля. Минуты числом человек не проверяет — 106 и
 * 66 выглядят одинаково правдоподобно, — а «1 ч 46 мин» рядом с тренировкой на
 * час видно сразу.
 */
export function describeDurationRu(minutes: number): string {
...
/**
 * Второе прочтение трёхзначного числа, если оно есть: «106» это и сто шесть
 * минут, и час шесть.
 *
 * ПОКАЗЫВАЕМ, НО НЕ ВЫБИРАЕМ. Выбрать за человека здесь нельзя: оба прочтения
 * встречаются, и оба правдоподобны. Наше дело — назвать оба и сказать, как
 * записать то, которое он имел в виду.
 *
 * null — двусмысленности нет: у двузначного её не бывает, а «160» не читается
 * как час шестьдесят.
 */
export function durationAlternativeRu(raw: string): string | null {
  const text = normalize(raw).replace(/(мин|минут|м|ч|час|часа|часов)$/iu, "");
  const digits = text.match(/^(\d)(\d{2})$/u);
  if (!digits) return null;
  const hours = Number(digits[1]);
  const minutes = Number(digits[2]);
  if (hours < 1 || minutes > 59) return null;
  return `Если это ${hours} ${hours === 1 ? "час" : "часа"} ${minutes} мин, напишите ${hours}:${digits[2]}`;
}
