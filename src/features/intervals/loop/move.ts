...
export function decideMove(input: {
  ...
  if (daysBetween(todayIso, session.sessionDate) < 0) {
    return {
      ok: false,
      code: "past_session",
      messageRu: "Прошедшую тренировку перенести нельзя. Если вы её пробежали, отметьтесь, если нет, просто идём дальше.",
    };
  }
  ...
  if (input.unavailableWeekdays.includes(toDayIdx)) {
    return {
      ...
      messageRu: `Вы отметили ${DAY_RU_FULL[toDayIdx]} как день, когда бегать не получается. Если это изменилось, скажите тренеру, поправим.`,
    };
  }
  ...
  if (taken) {
    return {
      ...
      code: "day_taken",
      messageRu:
        "В этот день уже стоит другая тренировка. Две подряд новичку не ставим, выберите свободный день.",
    };
  }
