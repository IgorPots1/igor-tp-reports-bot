...
const FACTOR_PATTERNS: Array<{ factor: StatedFactorKind; re: RegExp; neg?: RegExp }> = [
  ...
  // Влажность/духота — ОТДЕЛЬНО от жары: во влажном воздухе пот не испаряется, охлаждение хуже, пульс
  // выше при той же температуре (Черногория/лето — постоянная тема). Перекрытие с heat по «духот» — норм.
  { factor: "humidity", re: /влажн|духот|парн[оаяые]|парит|нет\s+ветра|воздух\s+сто[ия]|не\s+продува|нечем\s+дышать|сп[ёе]ртый\s+воздух|как\s+в\s+бане|парилк|марев/iu, neg: /не\s+влажн|сух[оаий]\s+воздух|продува|ветерок/iu },
...
const FACTOR_GROUNDING: Partial<Record<StatedFactorKind, RegExp>> = {
  ...
  soreness: /болит|колен|стоп|голеностоп|ахилл|сустав|спин|поясниц|бедр|икр|потянул|надорв|надрыв|мозол|прострел|ноет|ноют|тянет\s+(?:[а-яё]+\s+)?(?:мышц|ног|спин|бедр|икр|колен|стоп|сустав|поясниц)|(?:мышц|ног|бедр|икр|спин)[а-яё]*\s+тянет|травм|ушиб|растяж|заболел[аио]?\s+(?:колен|стоп|спин|ног|бедр|икр|сустав)/iu,
  ...
  undersleep: /недосып|недоспал|не\s+выспал|мало\s+спал|плохо\s+спал|не\s+спал|поздно\s+(?:лёг|лег|легл)|ночь\s+без\s+сна|бессон|глаза\s+слипал|разбит(?:ый|ая|о)?\s+с\s+утра/iu,
  dehydration: /вод[аыуе]|воды|пил|попи|питьё|пить\b|жажд|обезвож|сушняк|сухо\s+во\s+рту|пересох/iu,
  ...
  humidity: /влажн|духот|парит|парилк|нет\s+ветра|воздух\s+сто|не\s+продува|нечем\s+дышать|сп[ёе]рт|как\s+в\s+бане|марев/iu,
  life_stress: /(?:на|после|с|до|перед|из-за|по)\s+работ|работ[аеыу]\s+(?:завал|замот|замуч|вымот|доконал|съел|напряж|аврал|задолб|достал|много)|ремонт|переезд|аврал|завал|стресс|нерв|вымот|измотан|замот|запар|напряж|дедлайн|выгоран|операц|перегруз|устал|устаю|не\s+высыпаюсь|детьми|дети\s+(?:бол|леж)|откачива|командиров|смен[аеуы]\b/iu,
  conditions: /горк|в\s+гору|рельеф|подъём|подъем|дорожк|манеж|тредмил|бегов[а-яё]*\s+дорож|ветер|ветр|грязь|снег|гололёд|гололед|трасс|тропа|тропинк|асфальт|песок|набор\s+высот|горн|дожд|ливень|слякот|маршрут|обочин|набережн|лес\b|парк\b/iu,
  device_glitch: /час[ыои]|датчик|пульс|сердц|чсс|gps|джипиэс|спутник|навигац|трек|дистанц|расстоян|сигнал|глюч|вр[ёе]т|врал|сбо[йия]|кривой|крив|завыш|занижен|бар?ах|подвис|потерял/iu,
};
...
const FACTOR_ADVICE_KEY: Record<CauseFactorKind, AdviceKey> = {
  ...
  humidity: "cause_confirmed_humidity",
