...
async function generateNutritionWeeklyReviewNarrative(input: {
  ...
  const systemStable = [
    ...
    "ПРОДУКТЫ ИЗ ДНЕВНИКА — ЯЗЫК НЕ МЕНЯЙ: если название продукта в items_notable дано не по-русски (английское — так ученица сама записала в FatSecret) — пиши его В ТОЧНОСТИ КАК ДАНО, без перевода/транслитерации/русификации («Beef Stroganoff» остаётся «Beef Stroganoff», НЕ «говяжий строганов»; «Penne» остаётся «Penne», НЕ «пенне»). Правило «Пиши только на русском языке» — про твою прозу вокруг продукта, не про само название продукта: оно всегда цитируется как в дневнике. Из правила «упрости до бытового» это тоже исключение: бренд/магазин убрать можно, ЯЗЫК названия — нет.",
    ...
    "ИМЕНА ПОЛЕЙ — ВНУТРЕННЯЯ МАШИНЕРИЯ, УЧЕНИЦЕ НЕ ПОКАЗЫВАТЬ НИКОГДА: protein_advice_sources, items_notable, protein_contributor, carb_contributor, carb_class, checkin_numbers, previous_week_numbers, kcalTarget, proteinGMin, carbsGMin/carbsGMax и любое другое техническое имя поля из facts JSON (латиница со знаком подчёркивания или camelCase) — это твоя внутренняя опора для рассуждения, НЕ слова для текста. Если белковых продуктов в дне нет и код передал пустой protein_advice_sources — молча переходи к резервному списку: НЕ пиши «список пуст», «данных нет», «поле не заполнено» и НЕ называй имя поля ни в каком виде — просто по-доброму предложи один-два продукта, как свою обычную рекомендацию.",
