...
/* Нижняя граница разгона.
 *
 * Пять секунд — это не разгон, а округление. На 44:00 «наименьший разгон»
 * выдавал ровно ту же раскладку, что и «Ровный» (4:25 / 4:25 / 4:20), и кнопка
 * выглядела сломанной: человек её нажимает и ничего не меняется. С порогом в
 * десять секунд на той же цели находится точный вариант 4:20 / 4:30 / 4:15. */
const MIN_KICK = 10;
...
function shapeAllowed(paces: number[], finish: Finish): boolean {
  ...
  if (finish === "kick") {
    ...
    const kick = workingPace(paces) - paces[last];
    return kick >= MIN_KICK && kick <= MAX_KICK;
  }
