...
import { getClubRecords } from "@/features/club/service";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    const view = await getClubRecords({ currentStudentId: auth.student.id });
    return jsonResponse(200, { ok: true, view });
  } catch (error) {
    console.error("[m.club.records] failed", error);
    return jsonResponse(500, { ok: false, error: "Не удалось загрузить рекорды." });
  }
}
