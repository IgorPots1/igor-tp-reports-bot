...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  // ТРЕТЬЕ СООБЩЕНИЕ ЦЕПОЧКИ ЗАВЕДЕНИЯ [решение Игоря, 17.09.2026] — шлётся
  // ФАКТОМ ОТПРАВКИ АНКЕТЫ, а не нажатием тренера: раньше здесь была тишина,
  // и это ровно то место, где человек писал тренеру «а когда план?» сам,
  // не дождавшись ответа от бота. Best-effort, как и уведомление тренеру:
  // анкета уже сохранена, и сбой отправки не должен превращаться в ошибку API.
  await sendTelegramMessage(
    auth.telegramUserId,
    "Спасибо! Тренер посмотрит анкету и соберёт план. Когда он будет готов, сюда придёт " +
      "уведомление — заходить и проверять не нужно."
  ).catch((err) => {
    console.warn("[m.run.onboarding] student notify failed", { error: String(err) });
  });
